...
class TestLooksLikeGameId:
    def test_four_digits(self):
        assert _looks_like_game_id("1234") is True
        assert _looks_like_game_id("0012") is True

    def test_leading_zeros(self):
        assert _looks_like_game_id("0012") is True

    def test_too_short(self):
        assert _looks_like_game_id("123") is False

    def test_too_long(self):
        assert _looks_like_game_id("12345") is False

    def test_letters(self):
        assert _looks_like_game_id("1234a") is False
...
class TestClassifyMessage:
    def test_game_id_message(self):
        kind, data = _classify_message(_msg("1234"))
        assert kind == "game_id"
        assert data == "1234"

    def test_game_id_uppercased(self):
        # digits only so upper() has no effect, but ensure it returns stripped
        kind, data = _classify_message(_msg(" 0001 "))
        assert kind == "game_id"
        assert data == "0001"
